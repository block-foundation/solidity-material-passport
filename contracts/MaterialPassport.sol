...
contract MaterialPassport {
     ...
     * @param isRecyclable The isRecyclable
    ...
    modifier onlyOwner(uint _materialId) {
        require(
            materialOwners[_materialId] == msg.sender,
            "You are not the owner of this material"
        );
        _;
    }
    ...
    function recycleMaterial(
        uint _materialId
    ) public onlyOwner(
        _materialId
    ) {
        require(
            materials[_materialId].isRecyclable,
            "This material is not recyclable"
        );
        require(
            block.timestamp >= materials[_materialId].expirationDate,
            "This material has not yet expired"
        );
